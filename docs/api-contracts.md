...
**Manual web enrichment** is feature-flagged (`Workspace.leadEnrichment.enabled` + `OPENAI_API_KEY`). It never writes lead fields until the user accepts proposals. Accept uses repository `updateLead` (no campaign auto-enrollment). Search follows name, email, LinkedIn, and market (Swiss phone / CRM location / CHF). Ambiguous identity — including a hit in another country than the lead’s phone/market — returns no suggestions. Financial situation is a separate collection. Public-web search does not scrape personal finances; a labelled occupational pay estimate may pre-fill working figures (income midpoint, discussion budget) unless the source is already declared-by-lead / document / advisor. Contract: `/docs/lead-enrichment.md`.
