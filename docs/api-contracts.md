...
**Manual web enrichment** is feature-flagged (`Workspace.leadEnrichment.enabled` + `OPENAI_API_KEY`). It never writes lead fields until the user accepts proposals. Accept uses repository `updateLead` (no campaign auto-enrollment). Search nails the **project geo** first (`name + Geneva` for a Cressy / Genève lead), then email, LinkedIn, and phone/CHF market. Ambiguous identity — including a hit in another country than the project/phone market — returns no suggestions. Financial situation is a separate collection. Public-web search does not scrape personal finances; a labelled occupational pay estimate may pre-fill working figures (income midpoint, discussion budget) unless the source is already declared-by-lead / document / advisor. Contract: `/docs/lead-enrichment.md`.
...
**Step fields:** Each step has `order`, `delayDays`, `sendTime` (HH:mm, default `09:00`), `fromName` (optional; sender contact name shown in the inbox), `subject`, `body`. Campaign `senderName` / `defaultFromName` pre-fills new steps in the UI and is used at send time when a step `fromName` is empty or equals the campaign title (legacy bake). Updating the campaign sender contact name also rewrites steps that still use a blank from name, the campaign title, or the previous campaign sender name. Launch readiness requires an explicit campaign sender contact name. Sends and test emails never use the campaign title as the From name; missing contact names are skipped / rejected instead.
...
- **Pre-parse guards** (before `request.formData()`): `Content-Length` must not exceed **55 MB** (`5 × 10 MB screenshots + 5 MB form overhead`) → `VALIDATION_ERROR`; feedback rate limit (**10 / user / hour**) → `RATE_LIMITED` (429)
...
- `page_url` is optional; only `http`/`https` URLs whose origin matches `NEXT_PUBLIC_APP_URL` are stored — otherwise stored as `null` (admin UI renders untrusted legacy values as plain text, not clickable links)
- At least one of non-empty `body` or `screenshots` required
- Rate limit is enforced again in the service after parsing (defense-in-depth; single increment per successful submission)
