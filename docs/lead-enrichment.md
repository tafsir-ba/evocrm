...
3. Search is meant to work like a broker Googling the person: **name**, name+email, **LinkedIn**, then a **market** query (Swiss mobile `07…` / `+41`, CRM city/country that was not itself enrichment-written, or workspace `CHF` → Switzerland). Work-email domains also search `name + domain`. Contact-data vendors (RocketReach, ZoomInfo, …) are dropped. Org-chart scrapers rank below LinkedIn and employer pages. Distinctive names can match without the email appearing on the page. If the only hit is a person in another country than the lead’s phone/market (e.g. Montréal vs a Swiss `076` number), identity is **ambiguous** and nothing is written.
