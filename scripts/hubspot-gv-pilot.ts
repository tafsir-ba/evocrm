/**
 * Grosvenor Vistas first-write pilot runner.
 *
 * Defaults to dry-run. Writes require all three:
 *   --execute --confirm-write --manifest=<name>
 *
 * Usage:
 *   npm run migrate:hubspot-gv-pilot -- --manifest=gv-pilot-batch-01
 *
...
async function main(): Promise<void> {
  bootstrapEnv();
  const { runHubSpotGvPilot } = await import("../server/services/hubspot-gv-pilot");
  const report = await runHubSpotGvPilot({ argv: process.argv.slice(2) });
